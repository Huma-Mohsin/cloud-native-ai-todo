...
@router.get(
    "", response_model=list[TaskResponse], summary="Get all tasks for current user"
)
async def get_tasks(
    completed: bool | None = Query(
        None, description="Filter by completion status (true/false/null for all)"
    ),
    ...
    sort_by: str = Query(
        "created_at", description="Sort by: created_at, due_date, priority, position"
    ),
    user_id: int = Depends(get_current_user_id),
    session: AsyncSession = Depends(get_session),
):
...
@router.get("/today", response_model=list[TaskResponse], summary="Get tasks due today")
...
@router.get(
    "/upcoming", response_model=list[TaskResponse], summary="Get upcoming tasks"
)
...
@router.get("/export", summary="Export tasks as JSON or CSV")
...
@router.post("/bulk/positions", summary="Update positions of multiple tasks")
